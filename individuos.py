...
class Persona:
    ...
    def alta_persona(self, df_personas):
        ...
        if is_already:
            ...
            print(df_personas.loc[df_personas['Full Name']
            ...
            if proceed.lower() == 'y':
                ...
                nextStep = input(
                    'Desea asociar estos datos a un usuario existente?(Y/N)\n')
                if nextStep.lower() == 'y':

                    existing_id = int(input('Ingrese ID deseado  '))
                    self.numero_identificacion = existing_id

                    id_row = self.get_row_index_from_condition(
                        df_personas, 'id', existing_id)
    ...
    def get_row_index_from_condition(self, df, column_name, matching_value):
        ...
        row_ix = (np.arange(0, len(df), 1)[
                  df[column_name].values == matching_value])
        if len(row_ix) > 0:
            row_ix = int(row_ix)
        return row_ix
    ...
    def baja_persona(self, df_personas, df_usuarios, df_trabajadores):
        ...
        row_id_personas = self.get_row_index_from_condition(
            df_personas, 'id', self.numero_identificacion)
        row_id_trabajadores = self.get_row_index_from_condition(
            df_trabajadores, 'id', self.numero_identificacion)
        row_id_usuarios = self.get_row_index_from_condition(
            df_usuarios, 'id', self.numero_identificacion)
...
class Trabajador(Persona):
    ...
    def baja_trabajador(self, df_trabajadores):
        ...
        if df_trabajadores.columns[1] == 'Position':
            ...
            row_id = self.get_row_index_from_condition(
                df_trabajadores, 'id', self.numero_identificacion)
            # print(row_id)
...
class Usuario(Persona):
    ...
    def __init__(self, ocupacion, datos_persona):
        ...
        if type(datos_persona) == Persona:
            ...
            raise ValueError(
                ''' Accepted types for datos_persona are 'dict' and 'Persona' ''')
